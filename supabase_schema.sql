...
-- ─────────────────────────────────────────────────────────────
-- STORAGE — bucket "portfolio" + RLS policies
-- ─────────────────────────────────────────────────────────────
...
-- Leitura pública: visitantes carregam imagens via URL pública
create policy "portfolio_public_read"
  on storage.objects for select
  using ( bucket_id = 'portfolio' );

-- Upload: somente admin autenticado
create policy "portfolio_auth_insert"
  on storage.objects for insert
  to authenticated
  with check ( bucket_id = 'portfolio' );

-- Substituição de arquivo: somente admin autenticado
create policy "portfolio_auth_update"
  on storage.objects for update
  to authenticated
  using ( bucket_id = 'portfolio' );

-- Exclusão: somente admin autenticado
